...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # El path del archivo movie_descriptions con respecto a DjangoProjectBase sería:
        ...
        for i in range(100):
            movie = movies[i]
            exist = Movie.objects.filter(title=movie['title']).first()  # Se asegura que la película no exista previamente en la base de datos.
            if not exist:
